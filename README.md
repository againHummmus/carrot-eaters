...
Клод сохраняет рецепты в личную книгу пользователя через `save_recipe`; веб показывает их на `/recipes`.

Ключевое соглашение: **и ингредиенты, и КБЖУ хранятся в расчёте на ОДНУ порцию** (`recipes` +
`recipe_ingredients`, обе под RLS по `user_id` владельца). Число порций — это состояние интерфейса, а не
поле в БД: на странице рецепта переключатель порций домножает количества и нутриенты через
`scaleAmount`/`scaleNutrients` из `packages/shared`. У ингредиента `amount = null` означает «по вкусу» —
такие строки не масштабируются.

Инструменты MCP: `save_recipe` (перезаписывает рецепт с тем же названием), `list_recipes`, `get_recipe`
(с параметром `servings`), `delete_recipe` и `log_recipe_portion` — последний берёт КБЖУ порции прямо из
рецепта, умножает на `servings` и пишет обычный приём пищи в `meals`.
